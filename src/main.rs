...
fn main() {
    ...
    let legacy_addr = "1b1itzeSKYEKhdcthUSnNJ47Fx2U8Zwwn";
    let mut addr = Address::decode(legacy_addr).unwrap();

    // Change the base58 address to a test network earthaddr
    // addr.network = Network::Test;
    addr.scheme = Scheme::EarthAddr;
